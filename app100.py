...
class App:
    def __init__(self, master):
        ...
        self.host = int(config["ip_address"])
        self.port = int(config["port_number"])

        self.ip_label = tk.Label(master, text="IP Address:")
        self.ip_label.grid(row=0, column=0, padx=10, pady=10)

        self.ip_var = tk.StringVar(value=str(config["ip_address"]))
        ...
        self.port_var = tk.StringVar(value=str(config["port_number"]))
    ...
    def connect_tcpip(self):
        ...
        while True:
            ...
            if self.stop_button["state"] == tk.DISABLED:
                break
                # return False  # stop button has been pressed, exit early
            try:
